...
/// with level set values <= 0.5. The result is saved in the lsPointData of the lsDomain and
/// can be retrieved with lsDomain.getPointData().getVectorData("Normals").
...
#pragma omp parallel num_threads(levelSet->getNumberOfSegments())
    {
      ...
      std::vector<std::array<T, 3>> &normalVectors = normalVectorsVector[p];
    ...
    for (unsigned i = 1; i < levelSet->getNumberOfSegments(); ++i) {
      normalVectorsVector[0].insert(normalVectorsVector[0].end(), normalVectorsVector[i].begin(),
