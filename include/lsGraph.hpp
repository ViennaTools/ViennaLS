...
namespace lsInternal{
  ...
  class lsGraph {
    ...
    typedef typename std::set<std::size_t> edgeListType;
    // points must be unique and adressable
    typedef typename std::map<std::size_t, edgeListType> adjacencyListType;

    adjacencyListType adjacencyList;
    std::vector<int> componentIds;

    void depthFirstComponentSearch(const std::size_t &currentVertex, int &currentComponent) {
      ...
      if(vertexListIt == adjacencyList.end()) {
        lsMessage::getInstance().addError("lsGraph: Vertex " + std::to_string(currentVertex) + " could not be found although it should exist!");
      }

      for(auto it = vertexListIt->second.begin(); it != vertexListIt->second.end(); ++it) {
