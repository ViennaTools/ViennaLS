...
public:
  ...
  void apply() {
    ...
    for(unsigned i = 0; i < pointData.getScalarDataSize(); ++i) {
      ...
      mesh->pointData.insertNextScalarData(tmp, pointData.getScalarDataLabel(i));
    }
    // vector data
    for(unsigned i = 0; i < pointData.getVectorDataSize(); ++i) {
      VectorDataType tmp;
      tmp.reserve(pointData.getVectorData(i)->size());
      mesh->pointData.insertNextVectorData(tmp, pointData.getVectorDataLabel(i));
    }
        ...
        for (int n = 0; n < D; n++) {
          ...
          if (nodeIt != nodes[dir].end()) {
            ...
            for (unsigned i = 0; i < pointData.getScalarDataSize(); ++i) {
              ...
              mesh->pointData.getScalarData(i)->push_back(currentData[currentPointId]);
            }

            for (unsigned i = 0; i < pointData.getVectorDataSize(); ++i) {
              const auto &currentData = *pointData.getVectorData(i);
              mesh->pointData.getVectorData(i)->push_back(currentData[currentPointId]);
            }
          }
        }
