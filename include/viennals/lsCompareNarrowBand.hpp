...
/// Returns the sum of squared differences between corresponding grid points.
...
class CompareNarrowBand {
  ...
  bool checkAndCalculateBounds() {
    ...
    hrleIndexType targetMinX = gridTarget.isNegBoundaryInfinite(0) ? domainTarget.getMinRunBreak(0) : gridTarget.getMinIndex(0);
    hrleIndexType targetMaxX = gridTarget.isPosBoundaryInfinite(0) ? domainTarget.getMaxRunBreak(0) : gridTarget.getMaxIndex(0);
    hrleIndexType sampleMinX = gridSample.isNegBoundaryInfinite(0) ? domainSample.getMinRunBreak(0) : gridSample.getMinIndex(0);
    hrleIndexType sampleMaxX = gridSample.isPosBoundaryInfinite(0) ? domainSample.getMaxRunBreak(0) : gridSample.getMaxIndex(0);
    ...
    for (unsigned i = 0; i < D; ++i) {
      minIndex[i] = std::min({minIndex[i], 
                            (gridTarget.isNegBoundaryInfinite(i)) ? domainTarget.getMinRunBreak(i) : gridTarget.getMinIndex(i),
                            (gridSample.isNegBoundaryInfinite(i)) ? domainSample.getMinRunBreak(i) : gridSample.getMinIndex(i)});
                            
      maxIndex[i] = std::max({maxIndex[i], 
                            (gridTarget.isPosBoundaryInfinite(i)) ? domainTarget.getMaxRunBreak(i) : gridTarget.getMaxIndex(i),
                            (gridSample.isPosBoundaryInfinite(i)) ? domainSample.getMaxRunBreak(i) : gridSample.getMaxIndex(i)});
    }
...
public:
  ...
  /// Set whether to output squared differences (true) or raw differences (false)
  ...
  void apply() {
    ...
    std::unordered_map<hrleVectorType<hrleIndexType, D>, size_t, 
                       typename hrleVectorType<hrleIndexType, D>::hash> pointIdMapping;
      ...
      if (outputMesh != nullptr) {
        ...
        for (unsigned i = 0; i < (1 << D); ++i) {
          ...
          for (unsigned j = 0; j < D; ++j) {
            index[j] = itSample.getIndices(j) + itSample.getCorner(i).getOffset()[j];
        ...
        if (addVoxel) {
          if constexpr (D == 3) {
            ...
            std::array<unsigned, 4> quad{voxel[0], voxel[1], voxel[3], voxel[2]};
  ...
  /// Apply with mesh output - convenience method that ensures the mesh is generated
  void applyWithMeshOutput(SmartPointer<Mesh<T>> outputMesh, bool outputSquared = false) {
  ...
  T applyRMSE() {
    ...
    // Set up iterators for both level sets
    hrleConstDenseCellIterator<typename Domain<T, D>::DomainType> it(levelSetTarget->getDomain(), minIndex);
