      ...
      if constexpr (D == 2) {
        ...
        if (cornerIdx != -1) {
          ...
          if (std::abs(det) > 1e-3 && resF_yz.first && resF_xz.first &&
            ...
            auto addQuad = [&](unsigned a, unsigned b, unsigned c, unsigned d) {
              if (isConvex) {
        ...
        if (edgeAxis != -1) {
          ...
          auto checkSlice = [&](int sliceIdx, std::map<int, unsigned> &nodesMap)
              -> std::pair<bool, Vec3D<T>> {
            int corner = -1;
            for (int i = 0; i < 8; ++i) {
            ...
            if (std::abs(DotProduct(g1, g2)) < 0.98) {
              ...
              double det = g1[axes[0]] * g2[axes[1]] - g1[axes[1]] * g2[axes[0]];
              if (std::abs(det) < 1e-6)
                return {false, {}};

              double val1 = g1[axes[0]] * p1[axes[0]] + g1[axes[1]] * p1[axes[1]];
              double val2 = g2[axes[0]] * p2[axes[0]] + g2[axes[1]] * p2[axes[1]];
