...
template <int D>
void testInteriorDefinition(const std::string &suffix, size_t expectedBefore,
  ...
  viennahrle::ConstSparseStarIterator<
      typename Domain<NumericType, D>::DomainType, 1>
  ...
  while (itBefore.getIndices() < endIdx) {
    if (itBefore.getCenter().isDefined() &&
        itBefore.getCenter().getValue() < 0) {
      interiorBefore++;
    }
    itBefore.next();
  }
